...
class Main:
    def __init__(self, model, name_task, type_prompt=None, comp_model=None, X=None, vectorizer=None, task_lib=None):
        self.name_task = name_task
        self.type_prompt = type_prompt
        if name_task=='classification':
            self.task = Classification(model)
        elif name_task=='computation':
            self.task = Computation( model, comp_model, X, vectorizer, task_lib)
        elif name_task=='reasoning':
            self.task = Reasoning(model, comp_model, X, vectorizer, task_lib)
        else:
            self.task = QA_knowledge(model, comp_model, X, vectorizer, task_lib)
